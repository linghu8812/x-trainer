...
parser.add_argument("--policy_type", type=str, default="gr00tn1.5", help="Type of policy to use. support gr00tn1.5, lerobot-<model_type>, openpi, xtrainer_act, xtrainer_motus.")
...
parser.add_argument("--policy_timeout_ms", type=int, default=150000, help="Timeout of the policy server.")
...
def preprocess_obs_dict(obs_dict: dict, model_type: str, language_instruction: str):
    ...
    if model_type in ["gr00tn1.5", "lerobot", "openpi", "xtrainer_act", "xtrainer_motus"]:
...
def main():
    ...
    if args_cli.policy_type == "gr00tn1.5":
    ...
    elif args_cli.policy_type == "xtrainer_motus":
        # --------新增Motus HTTP客户端分支--------
        from leisaac.policy.service_policy_clients import MotusHttpClientPolicyClient
        from isaaclab.sensors import Camera
        model_type = "xtrainer_motus"
        policy = MotusHttpClientPolicyClient(
            host=args_cli.policy_host,
            port=args_cli.policy_port,
            timeout_ms=args_cli.policy_timeout_ms,
            camera_infos={key: sensor.image_shape for key, sensor in env.scene.sensors.items() if isinstance(sensor, Camera)},
        ...
        raise ValueError(f"Unsupported policy_type: {args_cli.policy_type}")
